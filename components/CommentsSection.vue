      ...
      comments.value?.map((c) => {
        if (c.id === comment.id) {
          const updatedComment = {
            ...
            dislikes: reaction === 1 ? c.dislikes?.filter((dislike) => dislike.profileId !== $authStore.profile?.id) : c.dislikes,
          };
...
<template>
  <div class="comments-sectio overflow-y-scrol absolute rounded-xl right-0 top-0 h-full w-[550px] z-20 bg-white dark:bg-neutral-900 shadow-lg">
    ...
    <div v-else class="p-4 overflow-y-auto h-[calc(100%-150px)]">
      <div v-if="comments?.length === 0" class="text-center text-gray-500 dark:text-gray-400 mt-4">No comments yet</div>
      <div v-else class="space-y-4">
        <div v-for="comment in comments" :key="comment.id" class="flex gap-3 relative">
          <NuxtLink :to="'/profile/' + comment.profile?.name">
            <img :src="'/upload/avatars/' + comment.profile?.avatar" class="w-10 h-10 rounded-full object-cover" :alt="comment.profile?.name" />
          </NuxtLink>
          <div class="flex-1 min-w-0">
            <div class="flex items-center gap-2">
              <NuxtLink :to="'/profile/' + comment.profile?.name" class="font-semibold dark:text-white hover:underline">{{
                comment.profile?.name
              }}</NuxtLink>
              <span class="text-gray-500 dark:text-gray-400 text-xs">{{ formatRelativeTime(comment.createdAt) }}</span>
            </div>
            <p class="text-gray-600 dark:text-gray-300 break-words whitespace-pre-wrap">{{ comment.text }}</p>
    ...
    <div class="absolute bottom-0 left-0 rounded-b-xl right-0 p-4 border-t dark:border-neutral-800 bg-white dark:bg-neutral-900">
