...
const props = defineProps<{ messages: IMessage[]; chatId: string; isTyping: boolean }>()
...
const filteredMessages = computed(() => {
	return props.messages.filter(
		(m) => !(m.text === "typing" && m.sender?.id === $authStore.user?.id)
	)
})
</script>
			...
			<div
				...
				<div class="flex items-start">
					<div
						...
						<div
							v-else
							class="w-full h-full bg-gray-300 flex items-center justify-center text-sm text-gray-600">
							{{ message.sender?.name }}
						</div>
					</div>
					...
					<div
						...
						<div
							...
							v-if="message.sender?.name && message.senderId !== $authStore.user?.id">
							{{ message.sender.name }}
						</div>

						<p v-if="message.text === 'typing'" class="typing-animation">
							{{ message.sender?.name }} is typing<span></span><span></span
							><span></span>
						</p>
