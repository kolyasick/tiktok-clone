...
<template>
  <div id="menu" class="bg-white/10 col-span-3 rounded-lg p-4">
    ...
    <div id="menu" class="flex flex-col space-y-2 my-5">
      <NuxtLink v-for="route in routes" :to="route.path" class="hover:bg-white/10 transition duration-150 ease-linear rounded-lg py-3 px-2 group">
        <div class="flex flex-col space-y-2 md:flex-row md:space-y-0 space-x-2 items-center">
          <Icon :name="route.icon" size="25" class="group-hover:text-indigo-400" />
          <div>
            <p class="font-bold text-base lg:text-lg text-slate-200 leading-4 group-hover:text-indigo-400">{{ route.name }}</p>
