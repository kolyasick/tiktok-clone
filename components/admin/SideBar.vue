...
<template>
  <div id="menu" class="bg-white/10 col-span-3 rounded-lg p-4">
    <NuxtLink to="/" class="font-bold text-lg lg:text-3xl bg-gradient-to-br from-white via-white/50 to-transparent bg-clip-text text-transparent">
    ...
    <ul id="menu" class="flex flex-col space-y-2 my-5">
      <li class="hover:bg-white/10 transition duration-150 ease-linear rounded-lg py-3 px-2 group" v-for="route in routes" :key="route.path">
      ...
      <li @click="$authStore.logout" class="hover:bg-white/10 transition duration-150 ease-linear rounded-lg py-3 px-2 group cursor-pointer">
        <div class="flex flex-col space-y-2 md:flex-row md:space-y-0 space-x-2 items-center">
    ...
    <p class="text-sm text-center text-gray-600">v2.0.0.3 | &copy; {{ new Date().getFullYear() }} Clipify</p>
  </div>
</template>
